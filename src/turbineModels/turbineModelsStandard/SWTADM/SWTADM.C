...
{   
    ...
    forAll(turbineName,i)
    {
        ...
        HubHt.append(scalar(readScalar(turbineArrayProperties.subDict(turbineName[i]).lookup("HubHt"))));
    ...
    */ // Read specific turbine properties
...
void SWTADM::computeWindVectors()
{
...
void SWTADM::computeBladeForce()
{
    ...
    forAll(windVectors, i)
    {
        ...
        powerRotor[i] = 0; // torqueRotor[i] * rotSpeed[i];

        // Compute the generator power.
        powerGenerator[i] = 0; //torqueGen[i] * (rotSpeed[i] * GBRatio[m]) * GenEfficiency[m];
    }
}


void SWTADM::computeBodyForce()
{  
...
void SWTADM::computeSectorAverage()
{
...
vector SWTADM::rotatePoint(vector point, vector rotationPoint, vector axis, scalar angle)
{
...
scalar SWTADM::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<scalar>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
label SWTADM::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<label>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
scalar SWTADM::compassToStandard(scalar dir)
{
    dir += 180.0;
    if (dir >= 360.0)
    {
...
scalar SWTADM::standardToCompass(scalar dir)
{
    dir = 90.0 - dir;
    if (dir < 0.0)
    {
