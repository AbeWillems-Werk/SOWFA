...
#include "simplifiedWindTurbinesADM.H"
...
simplifiedWindTurbinesADM::simplifiedWindTurbinesADM
(
...
void simplifiedWindTurbinesADM::rotateBlades()
{  
...
void simplifiedWindTurbinesADM::yawNacelle()
{
    // Perform rotation turbine by turbine.
    forAll(uvTower, i)
    {
...
void simplifiedWindTurbinesADM::computeRotSpeed()
{
...
void simplifiedWindTurbinesADM::filterRotSpeed()
{
    // Proceed turbine by turbine.
    forAll(rotSpeedF, i)
    {
...
void simplifiedWindTurbinesADM::controlGenTorque()
{
    // Proceed turbine by turbine.
    forAll(torqueGen, i)
    {
...
void simplifiedWindTurbinesADM::controlNacYaw()
{
    // Proceed turbine by turbine.
...
void simplifiedWindTurbinesADM::controlBladePitch()
{
    // Proceed turbine by turbine.
    forAll(pitch, i)
    {
...
void simplifiedWindTurbinesADM::findControlProcNo()
{
...
void simplifiedWindTurbinesADM::computeWindVectors()
{
...
void simplifiedWindTurbinesADM::computeBladeForce()
{
    ...
    forAll(windVectors, i)
    {
        ...
        // Set the total average velocity of the turbine to zero.  Average velocity will be summed on turbine-element-wise basis.
        VmagNAvg[i] = 0.0; 
        ...
        forAll(windVectors[i], j)
        {
            ...
            forAll(windVectors[i][j], k)
            {
                ...
                // Find the SECTOR AVERAGE velocity magnitude compose of only the axial flow
                VmagN[i][j][k] = windVectors[i][j][k].x(); 
                VmagNAvg[i] += VmagN[i][j][k] / (scalar(totDiskPoints[i])/scalar(nAvgSector[i]));
        ...
        // Proceed blade by blade.
        forAll(windVectors[i], jj)
        {

            // Proceed point by point.
            forAll(windVectors[i][jj], kk)
            {
                // Distribute this force uniformly along all blade force locations
                bladeForceSimplified[i][jj][kk] = TotalForceTurbineSimplified[i] / totDiskPoints[i] * bladeAlignedVectors[i][jj][kk][0];


                // Find the component of the blade element force/density in the axial (along the shaft)
                // direction.
                axialForceSimplified[i][jj][kk] = -bladeForceSimplified[i][jj][kk] & uvShaft[i];
                
                // Find the component of the blade element force/density in the tangential (torque-creating)
                // direction.
                tangentialForceSimplified[i][jj][kk] = bladeForceSimplified[i][jj][kk] & bladeAlignedVectors[i][jj][kk][1];

                // Add this blade element's contribution to thrust to the total turbine thrust.
                thrustSimplified[i] += axialForceSimplified[i][jj][kk] * solidity[i][jj];

                // Add this blade element's contribution to aerodynamic torque to the total turbine aerodynamic torque.
                torqueRotorSimplified[i] += tangentialForceSimplified[i][jj][kk] * solidity[i][jj] * bladeRadius[i][jj] * cos(PreCone[m][0]);
...
void simplifiedWindTurbinesADM::computeBodyForce()
{  
...
void simplifiedWindTurbinesADM::computeSectorAverage()
{
...
vector simplifiedWindTurbinesADM::rotatePoint(vector point, vector rotationPoint, vector axis, scalar angle)
{
...
scalar simplifiedWindTurbinesADM::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<scalar>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
label simplifiedWindTurbinesADM::interpolate(scalar xNew, DynamicList<scalar>& xOld, DynamicList<label>& yOld)
{
    label index = 0;
    label indexP = 0;
    label indexM = 0;
...
scalar simplifiedWindTurbinesADM::compassToStandard(scalar dir)
{
    dir += 180.0;
    if (dir >= 360.0)
    {
...
scalar simplifiedWindTurbinesADM::standardToCompass(scalar dir)
{
    dir = 90.0 - dir;
    if (dir < 0.0)
    {
...
void simplifiedWindTurbinesADM::update()
{
...
void simplifiedWindTurbinesADM::openOutputFiles()
{
    if (Pstream::master())
    {
...
void simplifiedWindTurbinesADM::printOutputFiles()
{
    if (Pstream::master())
    {
        forAll(alphaSecAvg,i)
...
void simplifiedWindTurbinesADM::printDebug()
{
...
volVectorField& simplifiedWindTurbinesADM::force()
{
